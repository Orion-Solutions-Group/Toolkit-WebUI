<template>
  <div v-if="isOpen" class="min-w-screen h-screen animated fadeIn faster fixed left-0 top-0 flex justify-center items-center inset-0 z-50" id="modal-id">
    <div class="absolute bg-black opacity-50 inset-0 z-0"></div>
    <div :class="modalSizeClass" class="p-5 relative mx-auto my-auto rounded-xl shadow-lg bg-white">
      <div>
        <div class="text-center p-5 flex-auto justify-center">
          <component :is="iconComponent" class="w-8 h-8 flex items-center mx-auto" :class="iconColorClass" />
        ...
        <div class="p-3 mt-2 text-center space-x-4 md:block">
          ...
          <button @click="confirmAction" :class="confirmButtonClass" class="mb-2 md:mb-0 px-5 py-2 text-sm shadow-sm font-medium tracking-wider text-white rounded-full hover:shadow-lg">
            {{ confirmText }}
          </button>
        </div>
      </div>
...
export default defineComponent({
  props: {
    ...
    message: {
      type: String,
      default: 'This is the regular modal with supporting text below as additional content.',
    },
    cancelText: {
      type: String,
      default: 'Cancel',
